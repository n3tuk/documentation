...
summary: |-
  Runbook for managing encryption-at-rest using LUKS via `cryptsetup`,
  specifically on bare-metal servers (the LVM which hosts the virtual machines
  is encrypted at rest, so individual virtual machhines do not need to be
  encrypted).
tags:
  - runbook
  - bare-metal
...
{{< alert "edit" >}} Provide an overview of the encryption-at-rest
configuration; noting it's general setup and purpose. {{< /alert >}}
